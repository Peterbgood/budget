import { useState, useEffect, useMemo } from 'react'; // Removed unused 'React' import to fix GitHub Build error
...
import { Trash2, Plus, Calendar, ReceiptText, Lock, PieChart, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-12">
        ...
        <div className="max-w-xl mx-auto bg-white rounded-3xl shadow-xl p-6 border border-white">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="text-right">
              ...
              <p className={`text-2xl font-black ${remaining < 0 ? 'text-red-500' : 'text-emerald-500'}`}>{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(remaining)}</p>
            </div>
          </div>
          
          {/* DISTRIBUTION BAR CHART (Inline Pie Replacement for Mobile Speed) */}
          <div className="mt-4">
             ...
             <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2">
                {chartData.slice(0, 4).map(([cat], i) => (
                  <div key={cat} className="flex items-center gap-1 text-[9px] font-bold text-slate-500 uppercase">
                    <div className={`w-2 h-2 rounded-full ${['bg-blue-500', 'bg-emerald-500', 'bg-orange-500', 'bg-purple-500'][i % 4]}`} />
                    {cat}
                  </div>
                ))}
             </div>
      ...
      <main className="max-w-xl mx-auto px-4 mt-8">
        ...
        <div className="space-y-3">
          ...
          {expenses.map(exp => (
            <div key={exp.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4 animate-in fade-in slide-in-from-bottom-2">
